...
上面展示出 `fread()` 存在众多的参数设置。R 的易用与友好在于此，众多 R 包提供的函数大多已经为用户恰当地设置好了默认选项，用户无需经常繁琐的操作即可快速获取所需的结果。

在学习了如何导入 TSV 文件后，我们应该能地想到 CSV、TSV 本质上是一类数据格式文件。例如，我们也可以使用分号 `;`（西方不少国家的逗号分隔文件就是以分号分隔的文件）。我们统一以 CSV 作为代表，将其他类似格式文件称为 CSV 变体。
...
R 中有诸多拓展多可以导入 Excel 中的数据，其中最为推荐的就是本部分介绍的 [**readxl**](https://github.com/tidyverse/readxl) 包。
